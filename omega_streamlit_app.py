...
ASSETS_DB = {"CRYPTO": {"BTC": "Bitcoin", "ETH": "Ethereum", "XRP": "Ripple", "SOL": "Solana", "ADA": "Cardano", "DOGE": "Dogecoin", "LINK": "Chainlink", "MATIC": "Polygon", "AVAX": "Avalanche", "POLKA": "Polkadot"}, "TECH": {"AAPL": "Apple", "MSFT": "Microsoft", "GOOGL": "Alphabet", "AMZN": "Amazon", "META": "Meta Platforms", "NVDA": "NVIDIA", "TSLA": "Tesla", "CRM": "Salesforce", "ADBE": "Adobe", "IBM": "IBM"}, "FINANCE": {"JPM": "JPMorgan Chase", "BAC": "Bank of America", "WFC": "Wells Fargo", "GS": "Goldman Sachs", "BLK": "BlackRock", "SCHW": "Charles Schwab"}, "HEALTHCARE": {"JNJ": "Johnson & Johnson", "UNH": "UnitedHealth", "PFE": "Pfizer", "ABBV": "AbbVie", "MRK": "Merck"}, "ENERGY": {"XOM": "ExxonMobil", "CVX": "Chevron", "MPC": "Marathon Petroleum", "COP": "ConocoPhillips"}, "CONSUMER": {"WMT": "Walmart", "KO": "Coca-Cola", "MCD": "McDonald's", "PG": "Procter & Gamble", "COST": "Costco"}, "INDICES": {"^GSPC": "S&P 500", "^IXIC": "NASDAQ", "^DJI": "Dow Jones", "^FTSE": "FTSE 100", "^FCHI": "CAC 40"}, "ETF": {"SPY": "S&P 500 ETF", "QQQ": "Nasdaq ETF", "IWM": "Russell 2000", "GLD": "Gold ETF", "TLT": "US Bonds"}}
...
def format_currency(v):
    try:
        return f"${float(v):,.2f}" if v and not pd.isna(v) else "$0.00"
    except:
        return "$0.00"
...
pv = data['Close'].iloc[-2] if len(data) > 1 else pc
pc_change = ((pc - pv) / pv * 100) if pv > 0 else 0

with col1:
    st.metric("Current Price", format_currency(pc), f"{pc_change:+.2f}%")

tc_change = ((metrics['theoretical_price'] - pv) / pv * 100) if pv > 0 else 0
with col2:
    st.metric("Theoretical Value (F1)", format_currency(metrics['theoretical_price']), f"{tc_change:+.2f}%")

with col3:
    st.metric("Spread (Δ) - F5", f"{metrics['spread']:+.2f}%", f"Signal: {engine.get_trade_signal(metrics['spread'])[1]}")
...
fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.12, row_heights=[0.7, 0.3], subplot_titles=("Price vs Theoretical Value (Formula 1)", "Shannon Entropy (Formula 2)"))

dates = data.index
prices = data['Close'].values
theo_values = metrics['theoretical_values']

fig.add_trace(go.Scatter(x=dates, y=prices, name="Market Price", line=dict(color="#0d9488", width=2.5), fill='tozeroy', fillcolor='rgba(13, 148, 136, 0.1)'), row=1, col=1)
fig.add_trace(go.Scatter(x=dates, y=theo_values, name="Theoretical Value", line=dict(color="#6b7280", width=2, dash='dash'), fill=None), row=1, col=1)

rolling_entropy = [engine.calculate_shannon_entropy(prices[max(0, i-29):i+1]) for i in range(len(prices))]
fig.add_trace(go.Bar(x=dates, y=rolling_entropy, name="Entropy", marker_color="#f59e0b"), row=2, col=1)

fig.update_layout(height=700, hovermode='x unified', template='plotly_white', font=dict(family="Inter, sans-serif", size=11, color="#6b7280"), title_text=f"{ticker} - Thermodynamic Analysis", showlegend=True)
...
def generate_ai_response(q):
    ...
    elif 'formula 1' in q or 'theoretical' in q:
        return f"**FORMULA 1 - V_t: {format_currency(metrics['theoretical_price'])}**\nCurrent: {format_currency(metrics['current_price'])}\nDiff: {metrics['spread']:+.2f}%"
    elif 'formula 3' in q or 'volume' in q or 'mass' in q:
    ...
    elif 'formula 5' in q or 'spread' in q or 'signal' in q:
        return f"**FORMULA 5 - Δ: {metrics['spread']:+.2f}%**\nSignal: {trade_signal}"
    elif 'buy' in q or 'compra' in q:
        if trade_signal == "BUY":
            return f"🟢 BUY! {ticker} is {abs(metrics['spread']):.2f}% undervalued"
        return f"❌ No BUY signal"
    elif 'sell' in q or 'vendi' in q:
        if trade_signal == "SELL":
            return f"🔴 SELL! {ticker} is {metrics['spread']:.2f}% overvalued"
        return f"❌ No SELL signal"
    else:
        return f"📊 F1: {format_currency(metrics['theoretical_price'])}\n🌪️ F2: {metrics['entropy']:.3f}\n⚖️ F3: {metrics['volume_multiplier']:.3f}x\n🔌 F4: {metrics['radar']:.4f}\n🎯 F5: {metrics['spread']:+.2f}%\n⚡ Signal: {trade_signal}"
...
with st.expander("📋 Price Data"):
    ...
    dd['Close Price'] = dd['Close Price'].apply(lambda x: format_currency(x))
