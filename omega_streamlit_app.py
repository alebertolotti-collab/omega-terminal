...
class ThermodynamicEngine:
    ...
    def add_data_point(self, price_series, volume_series=None):
        price_s = pd.Series(price_series).astype(float).dropna()
        if price_s.empty:
            return None

        prices = price_s.to_numpy(dtype=float)

        if volume_series is not None:
            volume_s = pd.Series(volume_series)
            if len(volume_s) != len(price_s):
                volume_s = pd.Series(np.ones(len(price_s)), index=price_s.index)
            volume_s = volume_s.astype(float).reindex(price_s.index).fillna(1.0)
            volumes = volume_s.to_numpy(dtype=float)
        else:
            volumes = np.ones(len(prices), dtype=float)
...
def format_currency(v):
    try:
        v = float(v)
        return f"${v:,.2f}" if np.isfinite(v) else "$0.00"
    except Exception:
        return "$0.00"
...
if metrics is None:
    st.error("Not enough clean data to compute metrics.")
    st.stop()
